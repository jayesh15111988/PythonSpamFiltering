...
def GenerateKMeansClusters(collectionOfVectorsOfAllMessages,numberOfDesiredOutputClusters):
    ...
    print("length of collection vector is Modified ** ",len(collectionOfVectorsOfAllMessages));
        ...
        for individualVector in collectionOfVectorsOfAllMessages:
            ...
            for outerIndex,individualCentroidHolderForInputMessages in enumerate(centroidHolderForInputMessages):
                for index,individualFeatureValueInFeatureVector in enumerate(individualCentroidHolderForInputMessages):
                    ...
                    euclideanDistanceFromCentroidForGivenVector=euclideanDistanceFromCentroidForGivenVector+(individualCentroidHolderForInputMessages[index]- individualVectorAttributeValue)**2       
    ...
    #Previous centroid that we had - Now calculate new centroid from the array we have and compare new centroid with older ones
        ...
        for index,individualCalculatedVector in enumerate(allMessagesContainer):
            tempHolderArrayForCentroids=[];
            lengthOfVector=len(individualCalculatedVector[0]);
            ...
            for indexLevelOne in range(0,lengthOfVector):
                ...
                for indexLevelTwo,individualPointVector in enumerate(individualCalculatedVector):
                    totalValue=totalValue+ individualPointVector[indexLevelOne] if indexLevelOne < len(individualPointVector) else -1 ;
        ...
        print("old centroid holder with points is ",centroidHolderForInputMessages);
        print("New centroid holder with points is ",newCentroidHolderForPoints);
        ...
        print("sum of diffrences is ",sumOfDifferencesForCentroids);
       
        
GenerateKMeansClusters([[1,2,3,4,5,6,8,9,0],[6,5,4,3,2,1],[4,5,6,3,2,1],[6,5,4,1,2,3],[100,200,300,456,432,291]],2);
